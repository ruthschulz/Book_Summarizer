...
from data_download_and_stats import first_lines_chapter, process_book, get_complete_summary_filename
from data_download_and_stats import get_key_concept_summary_filename, get_summary_filename, get_extractive_summary_filename
from data_download_and_stats import get_abstractive_summary_filename, get_abstractive_2_summary_filename
...
def summarize_book(book_id, num_chapters, args):
    ...
    if args.abstractive or args.combined:
        chapter_abstractive_summaries = create_abstractive_summary_book(
            book_id)
    if args.combined:
        create_complete_summary_book(
            book_id, num_chapters, chapter_abstractive_summaries)
...
def compare_summaries(book_id):
    ...
    analysis_data = [
        ['summary', 'spacy similarity', 'rouge_n', 'cosine similarity']]
    summary_doc, summary_sentences, summary_model = load_summary(
        get_summary_filename(book_id))
    key_concepts_doc, key_concepts_sentences, key_concepts_model = load_summary(
        get_key_concept_summary_filename(book_id))
    extractive_doc, extractive_sentences, extractive_model = load_summary(
        get_extractive_summary_filename(book_id))
    abstractive_doc, abstractive_sentences, abstractive_model = load_summary(
        get_abstractive_summary_filename(book_id))
    abstractive_2_doc, abstractive_2_sentences, abstractive_2_model = load_summary(
        get_abstractive_2_summary_filename(book_id))
    complete_doc, complete_sentences, complete_model = load_summary(
        get_complete_summary_filename(book_id))
    if summary_doc != '':
        if key_concepts_doc != '':
            analysis_data.append(['key concepts', summary_doc.similarity(key_concepts_doc), rouge_n(
                summary_sentences, key_concepts_sentences), cosine_similarity(summary_model, key_concepts_model)])
        if extractive_doc != '':
            analysis_data.append(['extractive', summary_doc.similarity(extractive_doc), rouge_n(
                summary_sentences, extractive_sentences), cosine_similarity(summary_model, extractive_model)])
        if abstractive_doc != '':
            analysis_data.append(['abstractive', summary_doc.similarity(abstractive_doc), rouge_n(
                summary_sentences, abstractive_sentences), cosine_similarity(summary_model, abstractive_model)])
        if abstractive_2_doc != '':
            analysis_data.append(['abstractive 2', summary_doc.similarity(abstractive_2_doc), rouge_n(
                summary_sentences, abstractive_2_sentences), cosine_similarity(summary_model, abstractive_2_model)])
        if complete_doc != '':
            analysis_data.append(['combined', summary_doc.similarity(complete_doc), rouge_n(
                summary_sentences, complete_sentences), cosine_similarity(summary_model, complete_model)])
...
def main():
    ...
    parser.add_argument(
        "-entity", help="create an entity summary of the book", action="store_true")
    parser.add_argument(
        "-extractive", help="create an extractive summary of the book", action="store_true")
    parser.add_argument(
        "-abstractive", help="create an abstractive summary of the book", action="store_true")
    parser.add_argument(
        "-abstractive2", help="create an abstractive summary of the book", action="store_true")
    parser.add_argument(
        "-combined", help="create a combined summary of the book", action="store_true")
    parser.add_argument(
