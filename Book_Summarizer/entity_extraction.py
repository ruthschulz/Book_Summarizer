...
def consolidate_list(long_list):
    sorted_long_list = sorted(long_list.items(), key=operator.itemgetter(1), reverse=True)
    matched_list = dict()
...
def find_entities_book(filename):
    ...
    for character in characters.keys():
        matched_character = find_matching_item(matched_entities.keys(), character)
        if matched_character in matched_characters:
            matched_characters[matched_character] = matched_characters[matched_character] + characters[character]
        elif len(matched_character)>0:
            matched_characters[matched_character] = characters[character]
    matched_characters, matched_entities = remove_characters_from_entities(matched_characters, matched_entities)
...
def find_entities_chapter(filename, book_characters, book_entities):
    ...
    for ent in doc.ents:
        ...
        matched_character = find_matching_item(book_characters.keys(), ent.text)
...
def create_sentence(entities, about_book=True, about_characters=True):
    ...
    num_to_print = (min(10, len(sorted_entities)-1) if about_book else min(5, len(sorted_entities)-1))
...
def test_entity_extraction_sentences():
    ...
    print(create_sentence(book_key_entities, about_book=True, about_characters=False))
    print(create_sentence(chapter_characters, about_book=False, about_characters=True))
    print(create_sentence(chapter_key_entities, about_book=False, about_characters=False))
