...
for t in range(4):
    ...
    for s in range(4):
        ...
        for d in set_of_documents[s]:
            print(d)
            parser_b = PlaintextParser.from_file(
                '../data/books/' + d, Tokenizer("english"))
            parser_s = PlaintextParser.from_file(
                '../data/summaries/' + d, Tokenizer("english"))
            ...
            scores[s].append([rouge_n_score, rouge_l_sentence_score,
                              rouge_l_summary_score, cosine_sim_score])
            print([rouge_n_score, rouge_l_sentence_score,
                   rouge_l_summary_score, cosine_sim_score])
